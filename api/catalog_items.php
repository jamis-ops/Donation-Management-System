...
$id = get_id_param();
$catalog = strtolower(trim((string) ($_GET['catalog'] ?? '')));
...
function ensure_catalog_table(PDO $pdo, string $table, array $defaults): void
{
  ...
  // Table names are from a fixed allow-list only.
  $pdo->exec("
...
if ($method === 'POST') {
  ...
  $row->execute([$newId]);
  json_response(['ok' => true, 'data' => map_catalog_item($row->fetch()), 'catalog' => $catalog], 201);
}
...
if ($method === 'PUT') {
  ...
  $row = $pdo->prepare("SELECT * FROM `{$table}` WHERE id = ?");
  $row->execute([$id]);
  json_response(['ok' => true, 'data' => map_catalog_item($row->fetch()), 'catalog' => $catalog]);
}
...
if ($method === 'DELETE') {
  ...
  $stmt = $pdo->prepare("SELECT id, label FROM `{$table}` WHERE id = ? LIMIT 1");
  $stmt->execute([$id]);
  $existing = $stmt->fetch();
  if (!$existing) {
    json_response(['ok' => false, 'error' => $meta['label'] . ' not found.'], 404);
  ...
  json_response(['ok' => true, 'data' => ['id' => (int) $id, 'label' => $existing['label']], 'catalog' => $catalog]);
}
