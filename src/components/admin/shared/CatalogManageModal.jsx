 ...
 * Rendered via portal so it never nests inside page forms (which broke saves).
 */
export default function CatalogManageModal({
  ...
  const publish = useCallback((list) => {
    ...
    notifyCatalogChanged(catalog, list)
  }, [catalog])
  ...
  const handleAdd = async () => {
    ...
    setBusy(true)
    try {
      const res = await catalogItemsApi.create(catalog, { label: next, catalog })
      if (!res?.ok && !res?.data) {
        throw new Error(res?.error || 'Save failed.')
      }
      setLabel('')
      notify.success(`“${next}” saved.`)
      await load()
    } catch (err) {
  ...
  return createPortal(
    <div className="admin-modal-overlay catalog-manage-overlay" onClick={onClose}>
      <div
        className="admin-modal admin-modal--wide catalog-manage-modal"
      ...
      >
        ...
        <div className="catalog-manage-modal__add">
          <input
            type="text"
            value={label}
            ...
            placeholder={`Add new ${String(title || '').replace(/^Manage\s+/i, '')}…`}
            maxLength={120}
            disabled={busy}
          ...
          <>
            <ul className="catalog-manage-modal__list" aria-label={title}>
              {seeMore.visible.map((row) => {
                ...
                return (
                  <li key={rowId} className="catalog-manage-modal__item">
                    {isEditing ? (
        ...
        {viewItem && (
          <div className="catalog-manage-modal__view" role="region" aria-label="Item details">
